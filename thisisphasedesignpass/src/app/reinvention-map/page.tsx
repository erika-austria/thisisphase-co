...
const ROOMS = [
  { num: 'I', label: 'Body', tint: 'pink', title: 'The PHASE™', line: 'The body that changed at forty-one. Perimenopause is the doorway, not the whole house.' },
  { num: 'II', label: 'Family', tint: 'navy', title: 'The Co-Parenting Power Method®', line: 'The blend and the co-parenting. The scripts for the messages you dread sending.' },
  { num: 'III', label: 'Voice', tint: 'pink', title: 'Self-Trust', line: 'Your own knowing, after a season of handing the microphone to everyone else.' },
  { num: 'IV', label: 'Work', tint: 'navy', title: 'Building Through It', line: 'Building something real while everything at home still needs you. Frameworks over willpower.' },
];
  ...
  return (
    <>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .rmap-room .rtile .rname{font-size:34px;}
                ...
                <article className="rmap-room" key={r.num}>
                  <div className="rtile">
                    <div className={`tile ${r.tint}`} role="img" aria-label={`The ${r.label} room`}>
                      <span className="rname">The {r.label}</span>
                    </div>
                  </div>
